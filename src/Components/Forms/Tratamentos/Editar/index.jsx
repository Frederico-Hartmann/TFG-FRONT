...
export default function FormEditarTratamento(props) {
  ...
  async function handleEdit(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.put(`/tratamentos/${idTratamento}`, data, {
          headers: {
  ...
  async function buscarTratamento() {
    handleOpen();
    try {
      const getTratamentoById = await api.get(`/tratamentos/${props.idTratamento}`, {
        headers: { Authorization: "Bearer " + getToken() }
      });
      handleClose();
      let dados = getTratamentoById.data[0];
      setIdTratamento(dados.id);
