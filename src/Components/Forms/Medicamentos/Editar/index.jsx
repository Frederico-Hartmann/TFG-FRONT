...
export default function FormEditarMedicamento(props) {
  ...
  async function handleEdit(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.put(`/medicamentos/${idMedicamento}`, data, {
          headers: {
  ...
  async function buscarMedicamento() {
    handleOpen();
    try {
      const getMedicamentoById = await api.get(`/medicamentos/${props.idMedicamento}`, {
        headers: { Authorization: "Bearer " + getToken() }
      });
      handleClose();
      let dados = getMedicamentoById.data[0];
      setIdMedicamento(dados.id);
